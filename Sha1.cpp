...
std::string CSha1Hash::AsString(eMethod method)
{
	...
	case REPORT_HEX_NOSPACE:
		{
			result.resize(40);
			for(int i = 0; i < 20; ++i)
				sprintf(&*(result.begin() + (i * 2)), "%02x", m_digest[i]);
		}
		break;

	case REPORT_HEX_DASHED:
		{
			result.resize(44);
			size_t place = 0;
			for(size_t i = 0; i < 20; ++i)
			{
				...
				sprintf(&*(result.begin() + place), "%02x", m_digest[i]);
				place += 2;
			}
		}
		break;
	...
	default:
		//ASSERT(FALSE);
		throw std::runtime_error("Error: Unknown report method!");
	}
...
void CSHA1::Final()
{
	...
	for (i = 0; i < 20; i++)
	{
		m_Hash.m_digest[i] = (unsigned char)((m_state[i >> 2] >> ((3 - (i & 3)) * 8) ) & 255);
	}
...
//Added by Johan
CSha1Hash CSHA1::GenerateHash(const char* P_szStringToHashPtr)
{
	return GenerateHash(P_szStringToHashPtr, (int)strlen(P_szStringToHashPtr));
}

CSha1Hash CSHA1::GenerateHash(const void* P_DataPtr, int P_iSize)
{
	string W_csReturn;
	CSHA1 sha1;

	sha1.Reset();
	sha1.Update((unsigned char *)P_DataPtr, P_iSize);
	sha1.Final();
...
CSha1Hash CSHA1::FromString(const char* hashStr)
{
	CSha1Hash result;
	
	size_t digestWalker = 0;
	char digit[3]; memset(digit, 0, sizeof(digit));
	for(const char* strWalker = hashStr; *strWalker != '\0'; ++strWalker)
	{
		char curr = *strWalker;
		if(isdigit(curr) || (curr >= 'a' && curr <= 'f') || (curr >= 'A' && curr <= 'F'))
		{
			...
			else
			{
				...
				if(digestWalker > 20)
					throw std::runtime_error(JStd::String::Format("Invalid sha1 hash. Too long: %s", hashStr));

				char* dummy;
				result.m_digest[digestWalker++] = (char)strtol(digit, &dummy, 16);
			...
			throw std::runtime_error(JStd::String::Format("Invalid sha1 hash. Unknown character %c. %s", curr, hashStr));
	}

	if(digestWalker < 19)
		throw std::runtime_error(JStd::String::Format("Invalid sha1 hash. Too short: %s", hashStr));
	
	return result;
}


bool CSHA1::IsValidHash(const char* hash)
{
	try
	{
		CSha1Hash(hash);
